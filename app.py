...
def signal():
    ...
    if risk_pips > MAX_RISK_PIPS:
        return jsonify({"status": "SKIPPED", "reason": f"Over {MAX_RISK_PIPS} pips", "symbol": symbol, "risk_pips": round(risk_pips,1), "send_to": PHONE_NUMBER})
    ...
    return jsonify({"status": "OK", "symbol": symbol, "risk_pips": round(risk_pips,1), "send_to": PHONE_NUMBER, "whatsapp_message": msg, "whatsapp_sent": bool(CALLMEBOT_APIKEY)})
